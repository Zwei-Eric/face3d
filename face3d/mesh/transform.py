...
def get_camera_matrix(s, R, t, fol):
    ''' get camera matrix from camera parameter
    
    Return: cm (3,4): camera matrix  
    '''
    C = np.zeros[3,3]
    C[0,0] = C[1,1]  = fol
    tm = np.zeros(3,4) 
...
def camera_calibration(vertices):
    ''' camera_calibration 
    reference: 3D shape Regression for Real-time Facial Animation.
    use binary search to optimise focal length, but mesh fitting estimate scale s, so don't need to do calibration to get 
    focal length, which uses to adjust subject's size.
    Arg   s:
        vertices: [nver, 3]. 
        
    Returns:
        op_fol: optimum focal length:
    '''
    # max focal length
    fol = 2000   
