...
def load_BSM(config_path):
    '''
    args:
           model_path: model core path, triangles indices path, 3d keypoint indices path
    return:
        model: (nver = 34530, ntri = 22800). nver: number of vertices. ntri: number of triangles.
            
    '''
    #core = np.fromfile(core_path, dtype = float)
    #core = base.reshape([-2, 150, 47])
    path = []
    with open(config_path) as config:
        for line in config.readlines():
            line = line.strip('\n')
            path.append(line)
    shapePC = np.loadtxt(path[0])
    shapeEV = np.loadtxt(path[1])
